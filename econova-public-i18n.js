(() => {
  ...
  const pairs = [
    ...
    ['Live Stream','البث المباشر'],['Follow live market sessions, commentary and context in a focused environment.','تابع جلسات السوق المباشرة والتعليق والسياق في بيئة مركّزة.'],['Daily Journal','السجل اليومي'],['Record daily trades or backtests quickly, keep context and build a useful performance record.','سجّل صفقاتك اليومية أو اختباراتك بسرعة، واحتفظ بالسياق وابنِ سجل أداء مفيد.'],['Decision Support','دعم القرار'],['Organize market context before execution without turning ECONOVA into a signal service.','نظّم سياق السوق قبل التنفيذ بدون تحويل ECONOVA إلى خدمة إشارات.'],['Market Intelligence','ذكاء السوق'],['Market structure, institutional positioning, macro events and AI context in one layer.','هيكل السوق، والتمركز المؤسسي، والأحداث الاقتصادية، وسياق الذكاء الاصطناعي في طبقة واحدة.'],['Explore →','استكشف ←'],
    ...
    ['Live sessions and market context','جلسات مباشرة وسياق السوق'],['Review and track your workflow','راجع سير عملك وتابعه'],['Organize context before execution','نظّم السياق قبل التنفيذ'],['Structure, macro, positioning and AI','الهيكل والاقتصاد والتمركزات والذكاء الاصطناعي']
  ];
  ...
  function translateRoot(lang){
    ...
    if (!root) return;
    root.querySelectorAll('[data-public-en]').forEach(el => {
      const en = el.getAttribute('data-public-en');
      el.textContent = lang === 'ar' ? (forward.get(en) || en) : en;
    });
    ...
    nodes.forEach(node => {
      ...
      if(!p.hasAttribute('data-public-original')) p.setAttribute('data-public-original', node.nodeValue);
      const original=p.getAttribute('data-public-original');
      const value=lang==='ar' ? (forward.get(original.trim()) || original.trim()) : original.trim();
      if(original.trim() && original.trim()!==value) node.nodeValue=original.replace(original.trim(),value);
      else if(lang==='en' && node.nodeValue!==original) node.nodeValue=original;
    });
    document.querySelectorAll('#publicLang').forEach(b=>b.textContent=lang==='ar'?'EN':'عربي');
  }

  function apply(){
    let lang='en'; try{lang=localStorage.getItem('habboub_language')==='ar'?'ar':'en';}catch(e){}
    document.documentElement.lang=lang;
    document.documentElement.dir=lang==='ar'?'rtl':'ltr';
    document.body.classList.toggle('econova-ar',lang==='ar');
    document.body.classList.toggle('econova-en',lang!=='ar');
    translateRoot(lang);
  }

  document.addEventListener('click',e=>{
    if(e.target.closest('#publicLang')){
      let current='en'; try{current=localStorage.getItem('habboub_language')==='ar'?'ar':'en';}catch(x){}
      try{localStorage.setItem('habboub_language',current==='ar'?'en':'ar');}catch(x){}
      apply();
    }
  });
  const boot=()=>apply();
