...
(function () {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      .hni-wrap{margin:0 0 22px;display:grid;gap:14px}.hni-command{display:flex;justify-content:space-between;gap:16px;align-items:center;padding:18px 20px;border:1px solid rgba(255,255,255,.09);border-radius:18px;background:linear-gradient(135deg,rgba(255,255,255,.045),rgba(255,255,255,.018));box-shadow:0 14px 40px rgba(0,0,0,.16)}.hni-command h3{margin:0 0 5px;font-size:18px}.hni-command p{margin:0;color:#8c9aaa;font-size:13px}.hni-live{display:flex;align-items:center;gap:8px;font-size:12px;font-weight:800;letter-spacing:.08em;white-space:nowrap}.hni-dot{width:8px;height:8px;border-radius:50%;background:#56e39f;box-shadow:0 0 12px rgba(86,227,159,.8)}
      .hni-assets{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.hni-asset{padding:14px;border:1px solid rgba(255,255,255,.08);border-radius:16px;background:rgba(255,255,255,.025)}.hni-asset-top{display:flex;justify-content:space-between;gap:10px}.hni-asset-name{font-weight:800}.hni-asset-symbol{font-size:10px;color:#788596;display:block;margin-top:3px}.hni-score{font-size:24px;font-weight:900}.hni-meter{height:7px;background:rgba(255,255,255,.08);border-radius:99px;margin:12px 0 8px;overflow:hidden}.hni-meter span{display:block;height:100%;border-radius:99px}.hni-note{font-size:11px;color:#8793a3;margin-top:4px}
      .hni-event{margin:10px 0 0;padding:14px 15px;border:1px solid rgba(255,255,255,.075);border-radius:15px;background:rgba(255,255,255,.018)}.hni-event-head{display:flex;align-items:center;justify-content:space-between;gap:12px}.hni-event-title{font-weight:800}.hni-badges{display:flex;gap:6px;flex-wrap:wrap}.hni-badge,.hni-severity-label{font-size:10px;font-weight:900;padding:4px 8px;border-radius:999px}.hni-badge{background:rgba(255,255,255,.07)}.hni-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin-top:12px}.hni-cell{padding:10px;border-radius:11px;background:rgba(255,255,255,.035)}.hni-cell small{display:block;color:#788596;font-size:10px;margin-bottom:4px}.hni-cell strong{font-size:13px}.hni-up{color:#56e39f}.hni-down{color:#ff7f86}.hni-pending{color:#ffc86b}.hni-foot{margin-top:7px;color:#6f7b8b;font-size:10px;line-height:1.5}
      .hni-severity-high{border-color:rgba(255,70,82,.58)!important;box-shadow:0 0 0 1px rgba(255,70,82,.08),0 12px 30px rgba(255,70,82,.10)}.hni-severity-medium{border-color:rgba(255,184,72,.52)!important;box-shadow:0 0 0 1px rgba(255,184,72,.07),0 12px 28px rgba(255,184,72,.08)}.hni-severity-low{border-color:rgba(82,221,145,.40)!important}.hni-severity-label.high{color:#ff6972;background:rgba(255,70,82,.10);border:1px solid rgba(255,70,82,.18)}.hni-severity-label.medium{color:#ffc45e;background:rgba(255,184,72,.10);border:1px solid rgba(255,184,72,.16)}.hni-severity-label.low{color:#67dfa0;background:rgba(82,221,145,.09);border:1px solid rgba(82,221,145,.14)}
  ...
  function statusText(event) {
    ...
    const total = Math.max(0, Math.floor((new Date(event.event_time).getTime() - Date.now()) / 1000));
  ...
  function directionCell(event, asset) {
    ...
    const up = p.up >= p.down, value = up ? p.up : p.down;
    return `<div class="hni-cell"><small>${asset.label}</small><strong class="${up ? "hni-up" : "hni-down"}">${up ? "↑" : "↓"} ${value}%</strong><div class="hni-foot">Confidence ${p.confidence ?? "--"}%${p.ai ? " · AI" : " · Consensus"}</div></div>`;
  }
  ...
  function buildTop() {
    const cards = ASSETS.map(asset => {
      ...
      return `<div class="hni-asset"><div class="hni-asset-top"><div><span class="hni-asset-name">${asset.label}</span><span class="hni-asset-symbol">${asset.symbol}</span></div><strong class="hni-score">${display}</strong></div><div class="hni-meter" style="color:${score === null ? "#6f7b8b" : score >= 50 ? "#56e39f" : "#ff7f86"}"><span style="width:${width}%;background:currentColor"></span></div><div class="hni-note">${note}</div></div>`;
    }).join("");
  ...
  function render() {
    ...
    if (!container) return;
    document.getElementById("habboubNewsIntelligence")?.remove();
